...
print("🚀 [39号] 启动多模态影像交集提取引擎 (切片级精确制导版)...\n")

# ================= 1. 加载映射底座 =================
print("📂 正在加载 [37号] 影像映射底座表...")
df_mapping = pd.read_excel(mapping_file)
df_mapping['MODALITYID'] = df_mapping['MODALITYID'].astype(str).str.strip()
df_mapping['病历号'] = df_mapping['病历号'].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
...
def extract_valid_modality_ids(clean_file, unmatched_file, modality_type):
    ...
    df_clean = pd.read_csv(clean_path, dtype=str)
    # 🌟 核心升级 1：构造切片级 UID (文件夹/文件名)
    if 'Image_Name' in df_clean.columns:
        df_clean['UID'] = df_clean['Image_Data_ID'].str.strip() + "/" + df_clean['Image_Name'].str.strip()
    else:
        df_clean['UID'] = df_clean['Image_Data_ID'].str.strip()
    
    # 🌟 核心升级 2：剔除 Unmatched 数据 (切片级精确剔除，绝不连坐)
    if os.path.exists(unmatched_path):
        df_unmatched = pd.read_csv(unmatched_path, dtype=str)
        if 'Image_Name' in df_unmatched.columns:
            df_unmatched['UID'] = df_unmatched['Image_Data_ID'].str.strip() + "/" + df_unmatched['Image_Name'].str.strip()
        else:
            df_unmatched['UID'] = df_unmatched['Image_Data_ID'].str.strip()
            
        invalid_uids = set(df_unmatched['UID'].dropna())
        df_clean = df_clean[~df_clean['UID'].isin(invalid_uids)]
        
    # 自动定位 Modality 列
    mod_col = [col for col in df_clean.columns if '(0008,0060)' in col.replace(' ', '') or 'Modality' in col][0]
    
    # 🌟 核心升级 3：正则强效清洗空格与特殊字符，防止静默漏判
    df_clean['Clean_Modality'] = df_clean[mod_col].astype(str).str.replace(r"[\[\]'\" ]", "", regex=True).str.upper()
    
    # 精确过滤
    if modality_type == 'CT':
        valid_df = df_clean[df_clean['Clean_Modality'] == 'CT']
    elif modality_type == 'MR':
        valid_df = df_clean[df_clean['Clean_Modality'] == 'MR']
    elif modality_type == 'XRay':
        valid_df = df_clean[df_clean['Clean_Modality'].isin(['DX', 'CR'])]
    else:
        valid_df = pd.DataFrame()
...
# ================= 3. 提取三大模态的底层 MODALITYID =================
print("🔍 正在穿透解析底层 DICOM 元数据，执行 UID 级精准扣减...")
ct_ids = extract_valid_modality_ids('10_CT_Metadata_Clean.csv', '22_New_Unmatched_CT_Metadata_Full.csv', 'CT')
mr_ids = extract_valid_modality_ids('10_MR_Metadata_Clean.csv', '22_New_Unmatched_MR_Metadata_Full.csv', 'MR')
xray_ids = extract_valid_modality_ids('10_XRay_Metadata_Clean.csv', '22_New_Unmatched_XRay_Metadata_Full.csv', 'XRay')
