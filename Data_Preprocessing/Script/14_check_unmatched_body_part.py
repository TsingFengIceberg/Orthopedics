...
def check_match(row, cols):
    ...
    for col in cols:
        ...
        if val == 'NAN' or val == 'EMPTY_VALUE':
            continue
        if any(kw in val for kw in WHITE_LIST):
            return True
    return False

print("🚀 开始反向筛选：提取【不符合】强脊柱约束的数据...\n")
...
for mod in modalities:
    ...
    # 导出不符合的数据表
    out_file = os.path.join(temp_dir, f"14_Unmatched_{mod}_Metadata_a.csv")
...
print("✅ 反向普查完毕！请检查不符合表中是否仍混有“含椎但未被识别”的漏网之鱼。")