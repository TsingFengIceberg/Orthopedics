...
# 注意：请确保这张 37 号表是最新生成的、包含最全文本报告的账单！
mapping_file = os.path.join(temp_dir, '37_Intersection_Image_Mapping.xlsx')
clinical_file = os.path.join(raw_tables_dir, 'Comprehensive_Clinical_Records.xlsx')
output_file = os.path.join(temp_dir, '41_Final_Timeline_Annotation_Template.xlsx')

print("🚀 [41号] 启动终极全景时间轴引擎 (全局首诊时序 + 切片级精准扣减版)...\n")
...
# 🌟 核心重构：植入【UID 切片级精确扣减法】
def get_survived_folders(mod_name):
    clean_file = os.path.join(temp_dir, f'10_{mod_name}_Metadata_Clean.csv')
    unmatch_file = os.path.join(temp_dir, f'22_New_Unmatched_{mod_name}_Metadata_Full.csv')
    
    if not os.path.exists(clean_file): 
        return set()
        
    # 读取 10 号表并构造 UID (文件夹ID/图片名)
    ...
    # 💥 执行切片级精准扣减，绝不连坐！
    df_survived = df_clean[~df_clean['UID'].isin(unmatched_uids)]
    
    # 返回依然有幸存切片的合法文件夹 ID 集合
    return set(df_survived['Image_Data_ID'].dropna().str.strip())

print("✂️  正在执行底层切片级黑名单精准扣减...")
ct_ids = get_survived_folders('CT')
mr_ids = get_survived_folders('MR')
xray_ids = get_survived_folders('XRay')

# 为原始文本账单打上清洗后的模态标签
df_img['Type'] = df_img['MODALITYID'].apply(lambda x: 'CT' if x in ct_ids else ('MR' if x in mr_ids else ('XRay' if x in xray_ids else 'Other')))
# 真正执行过滤：剔除 Other (即：要么在硬盘里找不到，要么被黑名单全部杀光的文件夹)
df_core_img = df_img[df_img['Type'] != 'Other'].copy()
print(f"   ▪ 成功保全有效影像记录: {len(df_core_img):,} 条。")
...
worksheet.set_column(1, 3, 30, wrap_format)  
worksheet.set_column(4, 4, 80, wrap_format)  
worksheet.set_column(5, 5, 20, wrap_format)  
...
print(f"\n🎉 完美收官！全网最严密的排版模板已生成：{output_file}")