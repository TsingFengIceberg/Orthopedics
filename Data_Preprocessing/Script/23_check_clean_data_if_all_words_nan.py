...
for mod in modalities:
    ...
    file_22 = os.path.join(temp_dir, f"22_New_Unmatched_{mod}_Metadata_Full.csv")
    
    if not os.path.exists(file_10) or not os.path.exists(file_22):
        print(f"⚠️ 找不到 {mod} 的相关原始/未匹配文件，跳过。")
        continue
        
    # 1. 加载数据
    df_10 = pd.read_csv(file_10, low_memory=False)
    df_22 = pd.read_csv(file_22, low_memory=False)
    
    # 2. 获取留存的正确数据 ( df_10 减去 df_22 )
    ...
    for k in keys:
        ...
        df_22[k] = df_22[k].astype(str)
        
    merged = pd.merge(df_10, df_22[keys], on=keys, how='left', indicator=True)
